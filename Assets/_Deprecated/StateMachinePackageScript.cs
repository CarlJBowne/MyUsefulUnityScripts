...
namespace StateMachineSLS
{
	...
	/// </summary>
	public abstract class StateMachine<TMachine, TOwner> where TMachine : StateMachine<TMachine, TOwner> where TOwner : MonoBehaviour
	{
		public TOwner owner;

		public static TMachine Create(TOwner owner)
		{
			TMachine machine = Activator.CreateInstance<TMachine>();
			machine.owner = owner;
